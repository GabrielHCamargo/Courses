    ...
    async with db as session:
        try:
            ...
            raise HTTPException(
                ...
                detail="ja existe um usuario com esse email cadastrado",
            )
...
async def put_usuarios(
    usuario_id: int, usuario: UsuarioSchemaUp, db: AsyncSession = Depends(get_session)
):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
...
async def delete_usuarios(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(UsuarioModel).filter(UsuarioModel.id == usuario_id)
        result = await session.execute(query)
...
async def login(
    ...
    if not usuario:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="dados de acesso incorreto"
        )
